...
# The replica ID is passed via environment variable
if [ -z "$REPLICA_ID" ]; then
  echo "ERROR: REPLICA_ID environment variable is not set"
  exit 1
fi
...
primary_conninfo = 'host=postgres-master port=5432 user=${REPLICATION_USER} password=${REPLICATION_PASSWORD} application_name=slave${REPLICA_ID}'
primary_slot_name = 'replica_slot_slave${REPLICA_ID}'
EOF
...
echo "Starting PostgreSQL as a standby server (Replica ${REPLICA_ID})..."
